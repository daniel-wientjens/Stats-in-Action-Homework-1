...
boxplot(CALCIUM ~ sex, data = data.study.p2, col = "blue")
```
...
Only then we conduct a t-test since we need to know whether to leave the var.equal input at it's default value or to change it to TRUE. 
```{r}
alpha<-0.05
...
```{r}
boxplot(CALCIUM ~ period, data = z, col = "blue")
```
```{r}
par(mfrow = c(1, 2))
hist(x$CALCIUM, main='Histogram of calcium for males P=1')
hist(y$CALCIUM, main='Histogram of calcium for males P=2')
```
As we can see the data seems to have a sort of normal distribution and hence a t-test could be justified.
...
```{r}
par(mfrow = c(1, 3))
hist(data.study.cont$CALCIUM, main='Histogram of calcium for males P=1')
hist(data.study.MON810$CALCIUM, main='Histogram of calcium for males P=2')
boxplot(CALCIUM ~ regimen, data = z, col = "blue")
```
...
par(mfrow = c(1, 2))
hist(dirbili.p2.f$DIRBILI, main='Histogram of DIRBILI for females')
hist(dirbili.p2.m$DIRBILI, main='Histogram of DIRBILI for male')
...
par(mfrow = c(1, 2))
hist(dirbili.p1.m$DIRBILI, main='Histogram of DIRBILI for males P=1')
hist(dirbili.p2.m$DIRBILI, main='Histogram of DIRBILI for males P=2')
```
...
par(mfrow = c(1, 2))
hist(dirbili.cont$DIRBILI, main='Histogram of DIRBILI for male control')
hist(dirbili.MON810$DIRBILI, main='Histogram of DIRBILI for male MON810')
```
