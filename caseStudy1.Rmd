...
```
Here we assume the variance is different for each group (aka the default setting of the t.test function). As we can see the p-value is very high, so we do not reject the H0, so the means are the same.
