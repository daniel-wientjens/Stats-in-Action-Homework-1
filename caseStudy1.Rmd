...
Then my first intuition would be to conduct a t-test since we need to know whether to leave the var.equal at it's default value or to change it to TRUE. 
```{r}
alpha<-0.05
...
But on second thought we should also run a Wilcoxon test since it is more powerfull due to lack of assumptions about the distribution of the data. 
```{r}
#add the wilcox test since we do not assume anything with it, when we do the t-test we assume the distribution
m <- data.study %>% filter(sex == 'M', period == 2) %>% select(CALCIUM) %>% na.omit()
f <- data.study %>% filter(sex == 'F', period == 2) %>% select(CALCIUM) %>% na.omit()
...
(The t-test assumes that the distribution of the data is normal, whereas with the wilcoxon we don’t make the assumption that the distribution of the data belongs to a family of parametric ditributions)
```{r}
m <- data.study %>% filter(sex == 'M', period == 1) %>% select(CALCIUM) %>% na.omit()
f <- data.study %>% filter(sex == 'F', period == 2) %>% select(CALCIUM) %>% na.omit()
x <- wilcox.test(m[,1], f[,1], alternative = 'two.sided', conf.level=1-0.05)
x
```
...
```{r}
m <- data.study %>% filter(sex == 'M', period == 2,regimen == 'control') %>% select(CALCIUM) %>% na.omit()
f <- data.study %>% filter(sex == 'M', period == 2,regimen == 'MON810') %>% select(CALCIUM) %>% na.omit()
x <- wilcox.test(m[,1], f[,1], alternative = 'two.sided', conf.level=1-0.05)
x
```
...
Hint: plot first the data and justify the test(s) to use.
   ...
   Here we unfortunately cannot do the same test as before since DIRBILI is categorical data
...
```{r}
table(dirbili.p2.m  %>% filter(regimen %in% c('control','MON810')) %>% select(regimen, DIRBILI))[1:2,]
```
```{r}
chisq.test(table(dirbili.p2.m  %>% filter(regimen %in% c('control','MON810')) %>% select(regimen, DIRBILI))[1:2,])
```
...
```{r}
#test if the mean level of calcium for period 2 is the same for males and females.  </br>**Hint:** plot first the data and justify the test(s) to use.
question.a <- function(variable){
  m <- data.study %>% filter(sex == 'M', period == 2) %>% select(variable) %>% na.omit()
  f <- data.study %>% filter(sex == 'F', period == 2) %>% select(variable) %>% na.omit()
  x <- wilcox.test(m[,1], f[,1], alternative = 'two.sided', conf.level=1-0.05)
  return(x$p.value)
}
question.a('CALCIUM')
...
```{r}
questionB <- function(variable, data){
  # Filter the variable for males in period 1 and 2
  dataNew <- data.study %>% filter(sex == 'M') %>% select(period, variable) %>% na.omit
...
questionB('CALCIUM', data.study)
```

```{r}
getp3 <- function(variable){
alpha=0.05
variable_period<-data.study %>% filter(period=='2',sex=='M') %>% select(variable,regimen) %>% na.omit()
x<-variable_period %>% filter(regimen=='control') %>% pull(variable)
y<-variable_period %>% filter(regimen=='MON810') %>% pull(variable) 
a=wilcox.test(x,y, alternative="two.sided", conf.level=1-alpha)
return(a$p.value)
}
getp3('CALCIUM')
```
...
```{r}
variable <- data.study  %>% select(-GLUCOSE, -BLOOD, -BILI, -PROT ,-KETONE, -UROBILI, -GAMMAGT, -TESTIS,-id,-X,-regimen,-sex,-period) %>% colnames() 
variable<- variable %>% as.data.frame()
